...
## 2026-09-06T11:51:47Z

최근 5단계 테스트 로드맵(2490개 vitest 통과, 55개 Playwright E2E 통과, 3x3 매트릭스, 22개 카오스 엔지니어링) 및 실서비스 환경 테스트 경험을 바탕으로, CRE IM 자동 생성 및 PPTX 발행 파이프라인 전 구간(외부 API, 브라우저 통신, LLM 엔진, 도메인 게이트, PPTX 렌더링)에서 발생 가능한 잠재적 장애 모드와 런타임 취약점을 전수 연구하고, 이를 방어하는 테스트 케이스 및 심층 연구 보고서를 작성하는 프로젝트.
...
### R1. 5-Layer 전 구간 잠재 취약점 및 장애 모드 심층 모델링
파이프라인 5대 레이어 전반을 체계적으로 감사하여 잠재적 런타임 오류 시나리오를 발굴한다:
- **L1 (외부 공공 API & 인프라 연동)**: JUSO/Kakao/V-World의 키 만료, 응답 구조 변동, 좌표 누락, 특수 지번(산, 블록, 가구), 다필지(multi-PNU) 불일치 시 장애 파급
- **L2 (클라이언트-서버 통신 & 세션 라이프사이클)**: 비동기 폴링 중 브라우저 백그라운드 전환, 네트워크 순단/지연, 해시 불일치, 대용량 다면 이미지 업로드 시 요청 유실/메모리 초과
- **L3 (IM Writer & LLM 런타임 예산)**: LLM 토큰 초과 절삭, JSON 파싱 실패, 수치 앵커 변조 시도, 180초 하드 타임아웃 예산 잠식 방어 실패
- **L4 (비즈니스 도메인 & 승인 게이트)**: 포스처별(5종) 필수 Claim 누락, 이상치(매매가 0원, 음수, 비정형 렌트롤) 유입 시 연산 예외, Grade D 차단 및 Tier 전파 결함
- **L5 (PPTX 물리 렌더링 & 지면 제약)**: Goldilocks 16면 절삭 시 슬라이드 누락, 텍스트 오버플로, 긴 매물명/테넌트명 줄바꿈 깨짐, 지도 이미지의 갤러리 침투

### R2. 잠재 취약점 방어 및 회귀 차단 테스트 케이스 작성
발굴된 주요 취약점 시나리오를 사전 검증하고 회귀를 영구 차단하기 위한 테스트 코드를 작성한다:
- `src/tests/e2e/` 또는 관련 테스트 디렉터리에 신규 방어 테스트 추가
- 모든 테스트는 Rule 7(Negative Pair 의무: 대조군 실패 검증)과 Rule 6(산출물 구조/수치/게이트 단언)을 철저히 준수
- 비동기 CPU 집약 렌더링 테스트의 경우 명시적 타임아웃(30s~60s, Rule 25) 적용

### R3. 종합 취약점 연구 및 대응 보고서 작성
연구 결과를 체계적으로 정리하여 `docs/audit/pipeline-vulnerability-research.md` 문서로 발행한다:
- 위험도(P0~P3) 및 레이어(L1~L5)별 취약점 분류 체계(Taxonomy)
- 각 오류의 발생 메커니즘, 실제 발현 조건, 비즈니스 및 사용자 경험에 미치는 영향
- 코드베이스 레벨의 선제적 완화(Mitigation) 전략 및 방어 아키텍처 제언

## Acceptance Criteria

### 취약점 연구 범위 및 깊이
- [ ] 5대 레이어(L1~L5) 전반에 걸쳐 최소 15개 이상의 구체적인 잠재 장애 시나리오가 보고서에 MECE하게 도출되어야 한다.
- [ ] 각 시나리오별로 위험도(P0/P1/P2/P3), 재현 조건, 영향도가 명확히 명시되어야 한다.

### 방어 테스트 무결성
- [ ] 식별된 주요 취약점을 검증하는 신규 테스트 케이스가 최소 10개 이상 작성되어야 한다.
- [ ] 모든 신규 테스트는 통과해야 하며, 기존 2490개 vitest 테스트와 55개 Playwright 테스트 스위트에 회귀가 없어야 한다 (`0 failed`).
- [ ] 모든 신규 테스트에 positive/negative 쌍이 존재해야 한다 (Rule 7).
