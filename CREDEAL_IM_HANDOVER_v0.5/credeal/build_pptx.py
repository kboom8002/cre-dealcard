...
from core import (ASSUMED_LOAN_RATE, MAN, PYEONG, eok, man, pct, sqm_pyeong)
...
C = {k: RGBColor(*D.rgb(k)) for k in
     ('ink', 'ink2', 'mute', 'navy', 'line', 'panel', 'paper',
      'gain', 'gainbg', 'loss', 'lossbg', 'seal', 'sealbg')}
C['white'] = C['paper']
C['warn'], C['warnbg'] = C['loss'], C['lossbg']
C['ok'], C['okbg'] = C['gain'], C['gainbg']
...
def txt(slide, x, y, w, h, text, *, size=12, bold=False, color='ink',
    ...
    # §4.7 한 줄 45자 이하는 **본문** 규칙입니다.
    # 캡션·꼬리말(8.5pt)까지 42자로 접으면 두 줄이 됩니다.
    wrap = 42 if size >= 11 else 84
    lines = []
    for raw in text.split('\n'):
        while len(raw) > wrap:
            cut = raw.rfind(' ', 0, wrap + 1)
...
def p01_cover(prs, core):
    ...
    txt(s, M, 3.60, CW, 0.34,
        ...
        f'{"공공데이터 결합 (R2판)" if core.has_public else "원장 기반 (R1판)"}',
...
LINE_H, GAP = 0.225, 0.11          # 11.5pt × 1.35 · 블록 간격
ROW_H, HEAD_H = 0.26, 0.29


def _lines(t: str, per: int = 42) -> int:
    ...
    for i, b in enumerate(blocks):
        ...
        if k in ('p', 'note', 'warn', 'h'):
            ...
            txt(s, M + (0.12 if k == 'warn' else 0), y,
                ...
                size=size, bold=bold, color=col, space=0)
...
PAGE_PLAN = [
    ...
    ('가격 근거',   'A03', 'price_basis',       None),
    ('가격 근거',   'A03', 'price_basis',       '가격 근거 (계속)'),
...
def photo_page(prs, core, which='외부·내부'):
    """사진이 0장이므로 한 면으로 통합합니다.

    정본 §4.1은 외부·내부 2면을 두지만, 미제출 안내를 두 번 반복하는 것은
    같은 정보를 두 면에 쓰는 것입니다. 사진이 들어오면 2면으로 나눕니다.
    """
    ...
    txt(s, M, 4.95, CW, 1.20,
        '표준은 외부·내부 사진 6장 이상을 필수로 둡니다.\n'
        '현장 사진은 매수인의 방문 결정에 가장 큰 영향을 줍니다.\n'
        '중개인이 직접 촬영한 사진으로 채우면 2개 면으로 나뉩니다.',
...
def build(core: im_core.IMCore) -> Presentation:
    ...
    photo_page(prs, core)                      # p9 — 사진 미제출 통합
    ns = secs['next_steps']
...
def main() -> int:
    for fid, name in (('dangsan', '당산동'), ('yangpyeong', '양평동')):
        for ed in ('R1', 'R2'):
            ...
            prs = build(core)
            p = OUT / f'{name}_PPTX_IM_{ed}.pptx'
            prs.save(str(p))
            print(f'{p.name:<28} {len(prs.slides):>2}장 · '
                  f'{p.stat().st_size:>7,} B · 해상도 '
                  f'{core.resolution_computed[0]}')
    return 0
