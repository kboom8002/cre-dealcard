...
┌─────────────────────────────────────────────┐
│  1) 도입부 (공통 4~6면)                       │
│     cover → [gallery] → summary → location   │
│     → land → building                        │
├─────────────────────────────────────────────┤
│  2) 본문 (포스처별 3~6면)                     │
...
| 순서 | dataKey | 아키타입 | 타이틀 | 데이터 소스 | 조건 |
|:---:|---|:---:|---|---|---|
| 1 | `cover` | A01 | 표지 | address, salePrice, hero/front 사진 | 필수 |
| 2 | `gallery` | A14 | 사진 | 물건 사진 1~4컷 | `hasPhotos=true` |
| 3 | `summary` | A02 | 핵심 요약 | 제원 6대 지표 + highlights 3~4건 | 필수 |
| 4 | `location` | A06 | 입지 분석 | 지도, 교통, 상권, 인프라 | 필수 |
| 5 | `land` | A04 | 토지 현황 | land_sqm, zoning, far, bcr, 공시지가 | 필수 |
| 6 | `building` | A04 | 건물 개요 | gfa_sqm, 구조, 층수, 준공일, 주용도 | 필수 |

> V5 감사 §3.1·§3.2 시정: `location`(입지)과 `gallery`(사진)를 공통 도입부에 명시.
> gallery는 `hasPhotos=true` 시에만 추가됩니다.
...
| **작성 요건** | ① 사진 없이 표지를 내지 않음 (im.pages.yaml §cover) ② **사진↔물건 결속 검증** — 사진이 해당 물건의 것인지 좌표/주소 대조 (D33 BL-B) |
| **게이트** | G01(가격), G02(면적), G03(주소), G20(사진PII), G31(DPI) |
...
| **작성 요건** | ① 지도 이미지 포함 시 min DPI 150 (capture) ② **지도 플레이스홀더 금지** — 실제 지도 이미지 없이 "지도 이미지가 표시됩니다" 등 placeholder 텍스트 삽입 금지 (D33 BL-E) |
| **게이트** | G31(DPI), G33(넘침) |
...
> V5 감사 B-11 시정: A11·A14·A18 추가 (19종 완전)
...
| A10 | 마감 | 면책 + 연락처 |
| A11 | 호실 스펙 | 호실별 면적/용도 |
| A12 | 실사 체크리스트 | 소유권/권리관계 — deck-sequencer `checklist` |
| A13 | KPI 대시보드 | 지표 카드 격자 |
| A14 | 사진 갤러리 | 1~4컷 배치 |
| A15 | 투자 논거 | 3대 포인트 서술 |
| A16 | 자본구조 | 에쿼티·레버리지 워터폴 |
| A17 | 스태킹 | 층별 면적 다이어그램 |
| A18 | 결손 이관 | BL-6 결손 항목 체크리스트 — `_deficiencies` 비어있지 않을 때 |
