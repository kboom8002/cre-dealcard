...
| **15** | **섹션 수를 늘리기 전에 생성 시간을 측정한다** (한계 120초) |
| **16** | **자동 comps 조회 불가 구간(주력 상단 300억 초과 · B4)은 `manual_comps` 없이 가격 근거를 제시하지 않는다** |
| **17** | **Golden 저장 전에 `sanitizePersona`·`stripMarkdown`을 실행한다** |
| **18** | **렌트롤은 전량 표기한다** (A03 8행 제한을 이유로 잘라내지 않는다) |
| **19** | **Hero 지표는 매수인이 검산 가능한 것으로 구성한다** (Cap Rate·WALE 금지) |
| **20** | **필수값 검증은 폼에서 먼저 하고 서버 게이트는 유지한다** (이중 방어) |
