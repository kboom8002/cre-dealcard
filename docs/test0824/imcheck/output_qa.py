...
# ── 금지어 (SSoT §7 · 부정문 문맥은 제외) ──────────────────────────────
BANNED = ['초역세권', '무결점', '극대화', '보장', '확정적', '최고', '유일',
          '우량', '급행', '제로', '완벽', '독보적', '희소성']
# 더 긴 낱말 안에 우연히 포함된 경우는 위반이 아니다.
# '실제로'·'전제로' 속의 '제로', '최고치' 속의 '최고' 등.
CONTEXT_EXCLUDE = {
...
NEGATION = ['기재하지 않', '쓰지 않', '붙이지 않', '금지', '판정하지 않',
            '산출하지 않', '넣지 않', '않습니다', '않았습니다', '않아', '없습니다',
            '아닙니다', '해서는 안', '금합니다']
...
DUP_METRICS = [
    ('QA-DUP-01', 'Cap Rate / 수익률',
     re.compile(r'(?:Cap\s*R(?:ate)?|캡\s*레이트|연\s*수익\s*률|순\s*수익\s*률'
                r'|Gross\s*Yield|총\s*수익\s*률)((?:(?!\s\s)[^%]){0,40}?)' + PCT)),
    ('QA-DUP-02', '실투자금',
     re.compile(r'실투자금(?![^억]{0,25}(?:대비|순수익))((?:[^억]){0,25}?)' + EOK_NUM)),
    ('QA-DUP-03', '자기자본수익률',
     re.compile(r'(?:자기\s*자본\s*수익\s*률|내\s*돈\s*대비[^%]{0,14}?수익\s*률|ROE)'
                r'((?:[^%]){0,30}?)' + PCT)),
    ('QA-DUP-04', '매매가',
     re.compile(r'(?:매매\s*(?:가|희망가)|매각\s*희망가)'
                r'(?![^억]{0,20}(?:대비|추가|차이))((?:[^억]){0,20}?)' + EOK_NUM)),
]
...
def check_finance(t: str, fx: dict, rep: Report):
    ...
    if exp.get('negativeLeverage'):
        roes = [float(m.group(2)) for m in DUP_METRICS[2][2].finditer(f)]
        pos = [v for v in roes if v > 0]
        if pos:
    ...
    if re.search(r'권역\s*평균|시세\s*대비|프리미엄|비교\s*사례', t):
