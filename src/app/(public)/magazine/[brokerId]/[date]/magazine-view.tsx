...
export function MagazineView({ data, brokerId, date, brokerVibe }: MagazineViewProps) {
  ...
  const renderBrokerProfile = () => {
    // Build FlatProfileCard props from brokerVibe (DB) + broker (magazine content)
    ...
    return (
      <div data-section-id="broker_profile" ref={sectionRef('broker_profile')}>
        <Section delay={0.35}>
          <FlatProfileCard
            name={broker.name ?? '중개인'}
            company={broker.company ?? undefined}
            specialty={[
              ...((bv?.specialty_regions as string[]) ?? (broker.specialtyRegions as string[]) ?? []),
              ...((bv?.specialty_assets as string[]) ?? (broker.specialtyAssets as string[]) ?? []),
            ].join(' · ')}
            photoUrl={broker.photoUrl ?? undefined}
            phone={broker.phone ?? undefined}
            slug={bv?.slug ?? brokerId}
        ...
        <div className="max-w-[440px] mx-auto flex gap-2">
          ...
          <button
            onClick={() => {
              ...
              if (slug) window.location.href = `/broker-profile/${slug}?ref=magazine-cta`;
            }}
