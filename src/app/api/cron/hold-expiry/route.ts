...
export async function GET(request: Request) {
  try {
    ...
    log.info("[Hold Expiry Cron] Starting slot hold expiry cleanup job...");

    const supabase = createServiceClient();
    const result = await expireHeldSlots(supabase);

    if (!result.success) {
      log.error("[Hold Expiry Cron] Process failed:", result.error);
    ...
    log.error("[Hold Expiry Cron] Unexpected error:", err);
    return NextResponse.json(
