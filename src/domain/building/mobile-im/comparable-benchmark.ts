 ...
 */
export function formatBenchmarkMarkdown(metrics: BenchmarkMetrics, compsCount: number, comps?: any[]): string {
  const statusEmoji =
  ...
  // 수동/자동 사례 구분 표시
  const manualCount = comps?.filter(c => c._isManual).length ?? 0;
  const autoCount = compsCount - manualCount;
