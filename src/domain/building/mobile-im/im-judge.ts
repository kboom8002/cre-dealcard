...
1. **factual_accuracy** (사실 정확도)
   - 제공된 SSoT/공공데이터/사용자 입력과 AI 서사의 수치가 일치하는가?
   - 건물 면적, 준공년도, 층수, 용도지역 등 팩트가 정확한가?
   - 사옥형(owner_occupied) 매물의 경우 사용자 메모/입력의 기업 조건(임직원 수, 현재 임차료, 자가전환 계획)은 정본 사실로 인정합니다.
...
3. **regulatory_compliance** (규제 준수)
   ...
   - "추천합니다", "수익이 보장됩니다", "법적 문제 없음" 등 금지 (단, 조건부 "절감 효과", "전환 검토" 등 사옥 분석 어휘는 허용)
   - 5점: 완벽 준수, 3점: 간접적 유도 표현 존재, 0점: 직접 금지 표현 포함

4. **investor_value** (투자자 가치)
   - 꼬마빌딩 매수자에게 유용한 정보 밀도가 충분한가?
   - 실행 가능한 인사이트(교통, 상권, 수익 구조, 사옥 활용 방안 등)가 포함되어 있는가?
...
5. **data_grounding** (데이터 근거)
   ...
   - 사옥형 공간 배분 및 임차료 vs 이자비용 비교 추정은 정당한 비즈니스 분석으로 인정합니다.
