...
이 건물은 매매가 450억 원 기준 연 수익률(Cap Rate) 2.5%~3.1%를 기대할 수 있는 안정적인 수익형 자산입니다. 대출과 보증금을 차감한 실투자금은 약 180억 원이며, 대출 활용 시 내 돈 대비 수익률은 6.3%~7.8%에 달해 우수한 자본 효율성을 기대할 수 있습니다. 평균 잔여 임기(WALE) 4.2년으로 임차 펀더멘털이 견고하며, 땅값 비중이 68.5%로 매우 높아 원금 하방 경직성도 우수합니다.
> ⚠️ 면책: 실제 수익은 임대차 계약 조건 및 세제에 따라 달라집니다.`,

  development: `[참고 예시 — 사업수지 분석 섹션]
본 건은 토지 평당 약 4,500만 원 수준에 매입이 가능하며, 249%의 용적률을 활용할 수 있는 뛰어난 개발 잠재력을 지녔습니다. 총 사업비는 약 120억 원으로 추정되며, 예상 개발 이익률은 18.5%에 이릅니다. 초기 브릿지 대출을 제외한 실투자금은 약 36억 원으로, 소규모 자본으로도 유의미한 개발 차익을 노려볼 수 있는 매력적인 기회입니다.
> ⚠️ 면책: 실제 개발이익은 공사비·인허가 및 분양 성패에 따라 상이합니다.`,

  operating: `[참고 예시 — 직영 운영 분석 섹션]
본 자산은 일일 객단가 15만 원, 가동률 75%를 안정적으로 달성 중인 고수익 운영 매물입니다. 연간 총매출 대비 35.7%의 우수한 마진을 시현하며, 실질 영업이익(GOP)은 약 12.5억 원에 달합니다. 매각가 300억 원 기준 GOP 환원율은 4.2%로 매력적이며, 선순위 대출 제외 시 약 120억 원의 자기자본으로 즉시 수익을 창출할 수 있습니다.
> ⚠️ 면책: 실제 GOP는 가동률 및 운영 비용 제어에 따라 변동됩니다.`,

  owner_occupied: `[참고 예시 — 사옥용 비용비교 섹션]
법인 사옥으로 직접 입주 시, 주변 임대 시세 대비 연간 약 3.2억 원의 임차 비용 절감 효과를 기대할 수 있습니다. 임대료 절감분을 통한 자가전환 손익분기 도달 기간은 약 7.5년이며, 평당 점유비용도 월 4.5만 원 수준으로 매우 경제적입니다. 약 45억 원의 실투자금만으로 안정적인 둥지를 마련하고 향후 시세차익까지 도모할 수 있습니다.
> ⚠️ 면책: 실제 절감액은 대출 조건 및 사옥 사용 면적에 따라 상이합니다.`,

  trading: `[참고 예시 — 시세 분석 섹션]
본 매물은 인근 유사 거래사례 평균 대비 12.5% 저평가된 4,200만 원/평에 매수할 수 있는 희소한 기회입니다. 2년 후 매각 시 목표 시세차익은 약 25억 원에 달할 것으로 추정되며, 약 30억 원의 초기 자본 투입으로 보유기간수익률(HPR) 35.2%라는 탁월한 자본 증식을 달성할 수 있습니다.
...
export function buildNarrativeUserPrompt(
  ...
  const sectionMission: Record<string, string> = {
    ...
    investment_thesis: "이 건물을 지금 사야 하는 '3대 핵심 투자 포인트'를 반드시 아래 형식으로 정확히 작성하세요:\n- **포인트 제목**: 구체적 수치와 근거를 포함한 상세설명\n- **포인트 제목**: 구체적 수치와 근거를 포함한 상세설명\n- **포인트 제목**: 구체적 수치와 근거를 포함한 상세설명\n각 포인트에는 반드시 검증 가능한 수치(㎡, 억 원, %, 도보 분 등)를 1개 이상 포함하세요. 마지막 줄에 '> **종합 가치 제안**: 자산의 안정성과 성장성을 겸비한 우량 부동산으로...' 형식의 실질적 투자 결론을 도출하세요.",
    ...
    land_detail: "토지 현황만 서술하세요: 용도지역·용도지구, 건폐율·용적률(법정 상한 vs 현재), 대지면적, 필지 형상(정형/부정형), 도로 접면(전면/측면/이면 접도), 지목, 개별공시지가. 임대차·렌트롤·임차인·보증금·월세·공실·실사·점검 등 임대 관련 내용은 절대 포함하지 마세요. 토지이용계획과 물리적 필지 특성에만 집중하세요.",
...
${fewShotBlock}`;
  ...
  prompt += `\n\n## [작성 요청]
위 데이터를 바탕으로 **${sectionType}** 섹션을 작성해 주세요. 어조는 단정적이고 소구력 높은 어조로 2~4문장의 줄글로만 작성하세요. (표 생성 절대 금지)`;
