...
export function buildDeckSequence(input: DeckSequenceInput): SlideSpec[] {
  ...
  switch (input.posture) {
    case 'income':
      // D41 B3: Comps는 hasComparables 플래그에 의해 조건부 추가 (Rule 9)
      const addComps = input.dataAvailability?.hasComparables === true;
  ...
  if (bodySlides.length > PAGE_RECOMMENDED) {
    ...
    // D45: building(건물 개요)은 basic-im-guide.md §2 필수 스펙이므로 보호 대상에 추가
    const protectedKeys = new Set(['cover', 'summary', 'closing', 'risk', 'checklist', 'process', 'thesis', 'location', 'building']);
