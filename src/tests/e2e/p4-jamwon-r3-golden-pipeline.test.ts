/**
 * @file p4-jamwon-r3-golden-pipeline.test.ts
 * @description P4 잠원 Dev R3-Verified
 */
...
const DATA_DIR = join(process.cwd(), 'docs', 'golden-test-data', 'p4-jamwon-dev', 'r3-verified');
const OUTPUT_DIR = join(process.cwd(), 'docs', 'test', 'golden-outputs', 'p4-jamwon-r3');
const PPTX_OUTPUT_PATH = join(OUTPUT_DIR, 'jamwon_dev_r3_basic.pptx');

describe('P4 잠원 Dev R3-Verified', () => {
  let bottomSheet: any;
  let pptxResult: any;
  ...
  it('Step 6: PPTX 렌더링', async () => {
    const pptxInput = await createBasicImTestInput(bottomSheet, { buildingName: '두원빌딩' });
  ...
  it('Step 8: 슬라이드 콘텐츠 검증', async () => {
    ...
    expect(fullText.includes('잠원')).toBe(true);
    expect(fullText.includes('242')).toBe(true);
  });
