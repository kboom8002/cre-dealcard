...
describe('Archetype A22: Architectural Setback Stacking Plan', () => {
  ...
  describe('Registry Registration', () => {
    it('[Positive] A22 아키타입이 SLIDE_ARCHETYPE_REGISTRY에 정상 등록되어 있어야 함', async () => {
    ...
    it('[Negative Pair] 존재하지 않는 A99 아키타입은 레지스트리에 없어야 함', async () => {
  ...
  describe('Tenant Category Inference', () => {
    ...
    it('[Positive] 주차장 및 기계실은 parking으로 분류되어야 함', async () => {
    ...
    it('[Positive] 공실 플래그 또는 공실 텍스트는 vacant로 분류되어야 함', async () => {
      expect(inferTenantCategory({ isVacant: true, tenant: '-' })).toBe('vacant');
      expect(inferTenantCategory({ tenant: '공실 (임차인 모집 중)' })).toBe('vacant');
    });

    it('[Negative Pair] 공실이 아니고 일반 업무인 경우 절대 vacant나 parking으로 잘못 분류되지 않아야 함', async () => {
  ...
  describe('A22 Slide Generation & Compliance', () => {
    ...
    it('[Positive] A22 정상 데이터 주입 시 슬라이드가 성공적으로 생성되어야 함', async () => {
      const input = {
        pres,
        slideNum: 5,
        docno: 'IM-TEST-2026',
    ...
    it('[Positive] 다크 모드(onDark: true)에서도 정상 렌더링되어야 함', async () => {
      const input = {
        pres,
        slideNum: 5,
        docno: 'IM-TEST-2026',
    ...
    it('[Negative Pair] Rule 1 페르소나 단어("60대 자산가") 오염 시 warnings 경고가 수집되어야 함', async () => {
      const input = {
        pres,
        slideNum: 5,
        docno: 'IM-TEST-2026',
  ...
  describe('Data Binder & Deck Sequencer Integration', () => {
    it('[Positive] buildA22Props가 마크다운 및 body에서 스태킹 데이터를 올바르게 구성해야 함', async () => {
      const body = {
    ...
    it('[Positive] bindSectionData가 lease_status로부터 stackingPlan 슬롯을 정상 합성해야 함', async () => {
      const doc = {
        title: 'NH농협캐피탈빌딩',
        body: {
          stackingPlan: nhCapitalStackingFixture,
    ...
    it('[Positive] deck-sequencer에서 hasStackingPlan: true일 때 A22 슬라이드가 편성되어야 함', async () => {
    ...
    it('[Negative Pair] hasStackingPlan이 없을 때는 income 시퀀스에 A22가 편성되지 않아야 함', async () => {
      const seq = buildDeckSequence({
        posture: 'income',
        grade: 'A',
        dataAvailability: {
