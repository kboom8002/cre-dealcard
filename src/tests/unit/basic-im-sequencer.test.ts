...
describe('Basic IM (credeal_basic) Sequencer Unit Test (Rule 47 & basic-im-guide.md)', () => {
  it('generates the exact standard 9-section Basic IM sequence for income posture', () => {
    ...
    // 6. 건물 사용 현황 (렌트롤 + 스태킹 플랜)
    expect(dataKeys[6]).toBe('rentRoll');
    expect(dataKeys[7]).toBe('stackingPlan');
    // 7. 투자수익률 분석 (A23)
    expect(dataKeys[8]).toBe('yieldFormula');
    // 8. 현장 사진 (A14)
    expect(dataKeys[9]).toBe('gallery');
    // 9. 문의 및 유의사항 (A10)
    expect(dataKeys[10]).toBe('closing');

    // Total: 11 slides (9 core sections + cadastral + stacking plan)
    expect(seq.length).toBe(11);

    // Negative Assertions: Pro-tier advanced slides must NOT be present
