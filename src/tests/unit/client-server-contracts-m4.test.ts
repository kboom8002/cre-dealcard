...
describe("Milestone 4 Client-Server Contracts", () => {
  it("F-2.2: generate/route.ts maxDuration should be 180s aligned with IM_HARD_TIMEOUT_MS", async () => {
    expect(maxDuration).toBe(180);
  });

  it("F-2.7: createMobileIMAction is exported and callable from actions.ts", async () => {
    expect(createMobileIMAction).toBeDefined();
    expect(typeof createMobileIMAction).toBe("function");
  });

  it("F-2.1: photo URL filtering logic properly eliminates empty, whitespace, and null values", async () => {
