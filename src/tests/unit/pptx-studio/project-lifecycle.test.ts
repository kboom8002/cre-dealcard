...
describe('PptxStudio Project Lifecycle & Optimistic Locking (PR-B3-01 / Negative-Pair Obligation)', () => {
  ...
  it('Positive Pair: Project creates with default slides and safely advances stages', async () => {
    const project = await service.createProject('deal-pptx-1', 'pkg-1', '강남 테헤란로 빌딩 투자안내서');
    expect(project.stage).toBe('S00_INIT');
    expect(project.slides.length).toBe(6);
    expect(project.lockVersion).toBe(1);

    // Update slide layout
    const updated = await service.updateSlideLayout(project.id, 2, 'A02_PHOTO_RIGHT', 1);
    expect(updated.lockVersion).toBe(2);
    expect(updated.slides[1].layoutType).toBe('A02_PHOTO_RIGHT');

    // Advance stage
    const advanced = await service.advanceStage(project.id, 'S10_COMPOSITION', 2);
    expect(advanced.stage).toBe('S10_COMPOSITION');
    expect(advanced.lockVersion).toBe(3);
  });

  it('Negative Pair: Stale lockVersion edit is rejected with STALE_LOCK_ERROR', async () => {
    const project = await service.createProject('deal-pptx-2', 'pkg-2', '여의도 사옥 매각안내서');
