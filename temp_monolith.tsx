...
export function ImDataBottomSheet({
  ...
  prefillLoanAmount,
  prefillVacancyPct,
  initialInvestmentPosture,  // 기본값 제거 — 중개인 필수 선택 (A-1)
  currentDataGrade,
  gradeUpItems,
  initialStage,
  targetTier = 'basic',
  ...
  // G-2: B1 FR20 수동 마스킹 확인 (온프레미스 AI 도입 전 임시 대응)
  const [photoMaskingConfirmed, setPhotoMaskingConfirmed] = useState(false);
  ...
  const computedMissingFields = React.useMemo(() => {
    ...
    // 0. 포스처 미선택 시 최우선 결손 — 다른 검증 불필요 (A-1)
      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto pr-2 space-y-6 mb-6 pb-10">
          ...
          <div>
            ...
            />
            {/* G-2: B1 FR20 수동 마스킹 확인 (온프레미스 AI 도입 전 임시 대응) */}
            {(existingUrls.length > 0 || photoFiles.length > 0) && (
              <label className="flex items-center gap-2 text-xs text-muted-foreground mt-3 p-2 bg-amber-50 dark:bg-amber-950/30 rounded-lg border border-amber-200 dark:border-amber-800">
                <input
                  type="checkbox"
                  checked={photoMaskingConfirmed}
                  onChange={(e) => setPhotoMaskingConfirmed(e.target.checked)}
                  className="rounded border-amber-300"
                />
                <span>간판·차량번호·얼굴 등 개인정보가 마스킹되었음을 확인합니다</span>
              </label>
            )}
          </div>
